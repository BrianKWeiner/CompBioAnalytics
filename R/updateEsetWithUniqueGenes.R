...
#' @return An updated ExpressionSet object with unique gene names and probes selected based on the highest median expression.
#' @importFrom Biobase ExpressionSet exprs featureData
#' @importFrom dplyr group_by summarise filter slice ungroup select
#' @export
updateEsetWithUniqueGenes <- function(eset) {
  ...
  exprData <- exprs(eset)
  # Extract gene symbols, taking the first symbol if multiple are present
  geneSymbols <- sapply(eset@featureData@data$`Gene Symbol`, function(x) strsplit(x, " /// ")[[1]][1])

  # Assuming 'geneInfo' contains columns 'geneSymbol' and 'medianExpr', and row names are probe IDs
